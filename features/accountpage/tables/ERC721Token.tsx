...
const ERC721Token: React.FunctionComponent<IERC721TokenProps> = ({
  account,
}) => {

    const {data, isLoading} = useQuery('Internal Transactions', async() => {
        const erc721TxResponse = await axios.get(`https://api.etherscan.io/api?module=account&action=tokennfttx&address=${account}&page=1&offset=100&startblock=0&endblock=27025780&sort=asc&apikey=${process.env.ETHERSCAN_API_KEY}`)
  ...
  return (
    <div>
     ...
     <div className='overflow-x-scroll'>
      ...
      <tbody>
      {data && data?.map((transaction: ERC721Transaction, i: number) => (
