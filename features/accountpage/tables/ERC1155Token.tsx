...
const ERC1155Token: React.FunctionComponent<IERC1155TokenProps> = ({
  account,
}) => {

  const {data, isLoading} = useQuery('Internal Transactions', async() => {
    const erc721TxResponse = await axios.get(`https://api.etherscan.io/api?module=account&action=token1155tx&address=${account}&page=1&offset=100&startblock=0&endblock=99999999&sort=asc&apikey=${process.env.ETHERSCAN_API_KEY}`)
    return erc721TxResponse.data.result
  })
     ...
     <div className='overflow-x-scroll'>
      ...
      <thead className='border-b'>
        <tr className='text-left text-sm tracking-wide'>
          ...
          <th className='px-3'>Transaction Info</th>
      ...
      <tbody>
      {data && data?.map((transaction: ERC1155Transaction, i: number) => (
